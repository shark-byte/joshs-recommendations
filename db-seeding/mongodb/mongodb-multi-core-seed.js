...
function seedDB(){
  MongoClient.connect('mongodb://localhost/').then((client) => {
    ...
    var count = parseInt(1000000 / numCPUs);
    var pageCounter = 0;
    const batchSize = 1000;
    const offset = num*count; 

    async function insertBulk(){
      const start = offset + pageCounter*batchSize;
      const end = offset + (pageCounter + 1)*batchSize;

      console.log('page', start, end, 'worker', num);

      const ops = _.range(start, end).map((id) => {
