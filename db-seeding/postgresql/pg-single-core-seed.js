...
function generateInsert(page, batchSize){
...
function generateTypeInsert(page, batchSize){
  const start = page*batchSize;
  const end = (page + 1)*batchSize;
...
function generatePhotoInsert(page, batchSize){
  const start = page*batchSize;
  const end = (page + 1)*batchSize;
...
async function seedDB(){
  ...
  try {
    ...
    async function insertRows(){
      ...
      const ins = generateInsert(pageCounter, batchSize);
      const typesIns = generateTypeInsert(pageCounter, batchSize);
      const photosIns = generatePhotoInsert(pageCounter, batchSize);
